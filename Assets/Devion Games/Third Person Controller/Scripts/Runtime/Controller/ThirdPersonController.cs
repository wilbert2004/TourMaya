...
namespace DevionGames
{
	public class ThirdPersonController : MonoBehaviour
	{
		...
		[SerializeField]
		private PhysicsMaterial m_IdleFriction;
		[SerializeField]
		private PhysicsMaterial m_MovementFriction;
		[SerializeField]
		private PhysicsMaterial m_StepFriction;
		[SerializeField]
		private PhysicsMaterial m_AirFriction;
		...
		public PhysicsMaterial IdleFriction {
			get { 
		...
		public PhysicsMaterial MovementFriction {
			get { 
		...
		public PhysicsMaterial StepFriction {
			get { 
		...
		public PhysicsMaterial AirFriction {
			get { 
		...
		private void FixedUpdate ()
		{
			...
			this.m_Velocity = this.m_Rigidbody.linearVelocity;
			if (this.IsGrounded) {
			...
			this.UpdateRotation ();
			this.UpdateAnimator ();
			this.m_Rigidbody.linearVelocity = this.m_Velocity;
		}
		...
		private void Footsteps(AnimationEvent evt) {
			...
			if (this.m_IsGrounded && m_Rigidbody.linearVelocity.sqrMagnitude > 0.5f && this.m_FootstepClips.Count > 0 && evt.animatorClipInfo.weight > 0.5f)
			{
